...
swagger_template = dict(
    info = {
        'title': LazyString(lambda:'API Documentation (Analisis Sentimen)'),
        'version': LazyString(lambda:'1.0.0'),
        'description': LazyString(lambda: 'API Analisis Sentimen (LSTM & RNN)')
    },
...
file_rnn = open('rnn_resources/x_pad_sequences.pickle','rb')
...
model_file_from_rnn = load_model('rnn_model/model.h5')
model_file_from_lstm = load_model('lstm_model/model.h5')
...
def rnn_text():
    ...
    json_response = {
        'status_code': 200,
        'description': 'Hasil Analisis Sentimen (RNN)',
        'data': {
            'text': original_text,
...
def lstm_text():
    ...
    json_response = {
        'status_code': 200,
        'description': 'Hasil Analisis Sentimen (LSTM)',
        'data': {
            'text': original_text,
            'sentiment': get_sentiment
        },
